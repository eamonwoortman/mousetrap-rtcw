...
// The following PunkBuster function computes an MD5 digital signature on a 
// specified file using the supplied seed. If size is given greater than 0, 
// then the first <size> bytes of the file are used for the computations.
...
// md5file
//
int md5file ( char *fn , unsigned long seed , MD5_CTX *mdContext , unsigned int size )
{
	FILE *f = fopen ( fn , "rb" ) ;
	if ( f == NULL ) return 0 ;		
